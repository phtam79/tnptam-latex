...
\newcommand{\khungbt}[1]{%
	\begin{tikzpicture}[baseline=(A.base),scale=1.2]%
	%	\node[diamond,draw=none,fill=violet,inner sep=1pt,outer sep=2pt] (A) {#1};
	\node[fill=violet!30,minimum width=1cm,minimum height=0.3cm,rounded corners=2mm] (A) {\color{blue}#1};
	\end{tikzpicture}%
}
\renewenvironment{bt}{
	\stepcounter{bt}
...
\fancyfoot[RE]{\color{violet}\bfseries\href{facebook.com/groups/toanvalatex}{facebook.com/groups/toanvalatex}~\color{blue}\faFacebookOfficial}
